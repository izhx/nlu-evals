...
#### https://huggingface.co/spaces/evaluate-metric/mse/blob/main/mse.py
...
class DataTrainingArguments:
    ...
    paired_texts: bool = field(
        default=False,
        metadata={
            "help": "Whether to tokenize texts as pairs."
        },
    )
...
def main():
    ...
    def preprocess_function(examples):
        if data_args.paired_texts:
            assert data_args.text_column_names is not None
            sentence1_key, sentence2_key = data_args.text_column_names.split(",")
    ...
    builtin_metrics = {'mse': Mse, 'accuracy': Accuracy, 'f1': F1}
    if data_args.metric_name is not None:
        assert data_args.metric_name in builtin_metrics
        # metric = (
        ...
        # )
        metric = builtin_metrics[data_args.metric_name](**{'config_name': "multilabel" } if is_multi_label else {})
        ...
        if is_regression:
            ...
            if is_multi_label:
                ...
                metric = F1(config_name="multilabel")
                logger.info(
    ...
    if training_args.do_predict:
        if data_args.test_config_names is None:
            ...
            if data_args.test_config_names == 'all_config':
            ...
            for config_name in config_names:
                if config_name == data_args.dataset_config_name:
                ...
                if is_regression:
                    ...
                    predictions = np.argmax(predictions, axis=1)
                output_predict_file = os.path.join(training_args.output_dir, f"predictions-{config_name}.txt")
                if trainer.is_world_process_zero():
                    with open(output_predict_file, "w") as writer:
                        logger.info("***** Predict results *****")
                        writer.write("index\tprediction\n")
            ...
            metric_name = f'predict_{data_args.metric_name}'
            if all(metric_name in m for m in all_metrics.values()):
                metrics[metric_name] = sum(m[metric_name] for m in all_metrics.values()) / len(config_names)
            trainer.save_metrics("predict", metrics)
