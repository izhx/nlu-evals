...
#### https://github.com/huggingface/transformers/blob/HEAD/examples/pytorch/text-classification/run_glue.py
...
####  https://huggingface.co/spaces/evaluate-metric/glue/blob/main/glue.py
...
def main():
    ...
    if is_regression:
        model.config.problem_type = "regression"  # Fix load ckpt from MNLI
    ...
    return_token_type_ids = getattr(config, 'type_vocab_size', 1) > 1
    ...
    def preprocess_function(examples):
        ...
        result = tokenizer(
            *args, padding=padding, max_length=max_seq_length, truncation=True, return_token_type_ids=return_token_type_ids
        )
    ...
    if data_args.task_name is not None:
        ...
        metric = CombinedEvaluations([Accuracy(), F1()])
