...
#### https://huggingface.co/spaces/evaluate-metric/squad/blob/main/squad.py
...
####  https://huggingface.co/spaces/evaluate-metric/squad_v2/blob/main/compute_score.py
import re
import string
...
#### https://huggingface.co/spaces/evaluate-metric/squad_v2/blob/main/squad_v2.py
...
class QuestionAnsweringTrainer(Trainer):
    ...
    def predict(
        ...
        post_process_prefix: str = 'predict'
    ):
        ...
        predictions = self.post_process_function(
            predict_examples, predict_dataset, output.predictions, post_process_prefix
        )
...
def main():
    ...
    if training_args.do_train:
        ...
        column_names = raw_datasets[data_args.test_split or 'test'].column_names
    ...
    if training_args.do_predict:
        if data_args.test_split is None and "test" not in raw_datasets:
            raise ValueError("--do_predict requires a test dataset")
        predict_examples = raw_datasets[data_args.test_split or 'test']
    ...
    # )
    metric = SquadV2() if data_args.version_2_with_negative else Squad()
    ...
    if training_args.do_predict:
        ...
        for config_name in config_names:
            ...
            if data_args.dataset_name is None or config_name == predict_dataset.config_name:
                test_ds, test_examples = predict_dataset, predict_examples
            else:
                test_examples = load_dataset(
                    data_args.dataset_name,
                    config_name,
                    split=data_args.test_split or 'test',
                    cache_dir=model_args.cache_dir,
                    token=model_args.token,
                    keep_in_memory=True,
                )
            ...
            results = trainer.predict(test_ds, test_examples, post_process_prefix=config_name)
        ...
        if data_args.test_config_names is not None:
            ...
            for k in ['test_f1', 'test_exact_match']:
                metrics[k] = sum(m[k] for m in all_metrics.values()) / len(config_names)
        trainer.save_metrics("predict", metrics)
